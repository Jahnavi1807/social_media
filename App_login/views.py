...
def sign_up(request):
	...
	return render(request,'App_login/signup.html',context={'title':'Sign_up Form Here','form':form})
...
# 	following_user = User.objects.get(username=username)
# 	follower_user = request.username
# 	already_followed = Follow.objects.filter(follower=follower_user,following=following_user)
...
def unfollow(request, username):
    following_user = User.objects.get(username=username)
    follower_user = request.user  # use request.user instead of request.username
    already_followed = Follow.objects.filter(follower=follower_user, following=following_user)
    already_followed.delete()
    return HttpResponseRedirect(reverse('App_login:user', kwargs={'username': username}))
